...
The installation script builds vLLM `0.29.0` from source with
`VLLM_TARGET_DEVICE=empty`.

To install against older supported vLLM versions `X.Y.Z` instead (e.g., `0.26.0`),
check out the `compat/vllm-X.Y.Z` tag (e.g., `compat/vllm-0.26.0`), the last plugin state
